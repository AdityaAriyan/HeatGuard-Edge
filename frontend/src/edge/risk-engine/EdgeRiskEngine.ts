...
import { EdgeSensorPacket } from '../sensors/interfaces';

export interface UserBaseline {
  userId: string;
  baselineHeartRate: number; // e.g. 72 BPM
  baselineSpo2: number; // e.g. 98%
  baselineSkinTemp: number; // e.g. 36.4 °C
  baselineSystolic: number; // e.g. 118 mmHg
  baselineDiastolic: number; // e.g. 78 mmHg
  typicalSleepMinutes: number; // e.g. 450 (7.5h)
  calibratedAt?: string;
}

export type RiskLevel = 'LOW' | 'CAUTION' | 'HIGH' | 'CRITICAL';

export interface RiskSubVector {
  score: number; // 0 - 100
  level: RiskLevel;
...
export interface RiskAssessmentResult {
  ...
  risks: {
    ...
    fallEmergency: number;
  };
  levels: {
    heat: RiskLevel;
    dehydration: RiskLevel;
    respiratory: RiskLevel;
    fatigue: RiskLevel;
    cardiovascular: RiskLevel;
    fallEmergency: RiskLevel;
  };
  baselineDeltas: {
  ...
  isFallSuspected: boolean;
  requiresImmediateSos: boolean;
  computedAt: string;
  engineLocation: 'EDGE_DEVICE_LOCAL' | 'BACKEND_CLOUD';
  disasterContext?: string;
}

export class EdgeRiskEngine {
  /**
   * Helper to map a numeric 0-100 score to standardized prototype risk level
   */
  public static getLevel(score: number): RiskLevel {
   ...
   */
  public static calculateHeatIndex(tempC: number, humidity: number): number {
  ...
  public static calculateHeatRisk(
    ...
    if (heatIndex >= 45) score += 40;
    else if (heatIndex >= 40) score += 32;
    else if (heatIndex >= 35) score += 22;
    else if (heatIndex >= 30) score += 12;
    else score += 4;

    // Skin Temperature Deviation (max 25 pts)
    const skinDelta = packet.skinTemperature - baseline.baselineSkinTemp;
    if (skinDelta >= 1.8) score += 25;
    else if (skinDelta >= 1.2) score += 18;
    else if (skinDelta >= 0.6) score += 10;
    else if (skinDelta >= 0.2) score += 4;

    // Heart Rate Elevation in Heat (max 20 pts)
    const hrDelta = packet.heartRate - baseline.baselineHeartRate;
    if (hrDelta >= 35) score += 20;
    else if (hrDelta >= 22) score += 14;
    else if (hrDelta >= 12) score += 8;

    // Exposure Duration Factor (max 15 pts)
    if (packet.exposureMinutes >= 180) score += 15;
    else if (packet.exposureMinutes >= 120) score += 10;
    else if (packet.exposureMinutes >= 60) score += 5;
   ...
   */
  public static calculateDehydrationRisk(
    packet: EdgeSensorPacket,
    baseline: UserBaseline,
    ...
    if (hrDelta >= 30) score += 35;
    else if (hrDelta >= 18) score += 22;
    else if (hrDelta >= 8) score += 10;
    ...
    if (packet.exposureMinutes >= 90) score += 15;
    else if (packet.exposureMinutes >= 45) score += 8;
  ...
  public static calculateRespiratoryRisk(
    ...
    if (packet.aqi >= 300 || packet.pm25 >= 250) score += 45; // Hazardous
    else if (packet.aqi >= 200 || packet.pm25 >= 150) score += 35; // Very Unhealthy
    else if (packet.aqi >= 150 || packet.pm25 >= 55) score += 25; // Unhealthy
    else if (packet.aqi >= 100 || packet.pm25 >= 35) score += 15; // Moderate
    else score += 4;

    // SpO2 Deviation from Personal Baseline (max 40 pts)
    const spo2Delta = baseline.baselineSpo2 - packet.spo2; // positive if dropped
    if (packet.spo2 < 90 || spo2Delta >= 6) score += 40;
    else if (packet.spo2 < 93 || spo2Delta >= 4) score += 30;
    else if (packet.spo2 < 95 || spo2Delta >= 2) score += 18;
    else if (spo2Delta >= 1) score += 8;

    // Respiratory compensation: elevated HR during poor air
    const hrDelta = packet.heartRate - baseline.baselineHeartRate;
    if (hrDelta >= 20 && packet.aqi > 150) score += 15;
   ...
   */
  public static calculateFatigueRisk(
    packet: EdgeSensorPacket,
    baseline: UserBaseline
  ): number {
    let score = 0;
    ...
    if (sleepDeficitMinutes >= 180) score += 45; // Missing 3+ hours
    else if (sleepDeficitMinutes >= 120) score += 30;
    else if (sleepDeficitMinutes >= 60) score += 15;

    // Continuous exposure / exertion (max 30 pts)
    if (packet.exposureMinutes >= 240) score += 30;
    else if (packet.exposureMinutes >= 150) score += 18;
    else if (packet.exposureMinutes >= 60) score += 8;
   ...
   */
  public static calculateCardiovascularStress(
    packet: EdgeSensorPacket,
    baseline: UserBaseline
  ): number {
    let score = 0;
    ...
    if (packet.heartRate >= 130 || hrDelta >= 45) score += 40;
    else if (packet.heartRate >= 110 || hrDelta >= 28) score += 28;
    else if (packet.heartRate >= 95 || hrDelta >= 15) score += 14;

    if (packet.systolic >= 160 || sysDelta >= 35) score += 35;
    else if (packet.systolic >= 140 || sysDelta >= 20) score += 22;
    else if (packet.systolic >= 130 || sysDelta >= 10) score += 10;

    if (packet.diastolic >= 95) score += 15;
    else if (packet.diastolic >= 85) score += 8;
   ...
   */
  public static calculateFallRisk(packet: EdgeSensorPacket): { score: number; isFall: boolean } {
    ...
    const isFallDetected =
      packet.activityState === 'POSSIBLE_FALL' ||
  ...
  public static assessRisk(
    packet: EdgeSensorPacket,
    baseline: UserBaseline,
    disasterContext: string = 'NONE',
    engineLocation: 'EDGE_DEVICE_LOCAL' | 'BACKEND_CLOUD' = 'EDGE_DEVICE_LOCAL'
