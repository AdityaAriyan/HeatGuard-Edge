...
exports.getPersonalDashboard = getPersonalDashboard;
exports.getCaregiverDashboard = getCaregiverDashboard;
const db_1 = require("../database/db");
async function getPersonalDashboard(req, res) {
    try {
        const userId = req.user.id;
        // 1. Fetch User Profile & Baseline
        const user = await (0, db_1.queryRow)('SELECT id, email, full_name, role, phone FROM users WHERE id = ?', [userId]);
        const profile = await (0, db_1.queryRow)('SELECT * FROM user_profiles WHERE user_id = ?', [userId]);
        const baseline = await (0, db_1.queryRow)('SELECT * FROM baselines WHERE user_id = ?', [userId]);
        // 2. Fetch Latest Sensor Reading
        const latestReading = await (0, db_1.queryRow)('SELECT * FROM sensor_readings WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [userId]);
        // 3. Fetch Latest Risk Assessment
        const latestRisk = await (0, db_1.queryRow)('SELECT * FROM risk_assessments WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [userId]);
        // 4. Fetch Latest Location
        const latestLocation = await (0, db_1.queryRow)('SELECT latitude, longitude, accuracy, timestamp FROM location_records WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [userId]);
        // 5. Fetch Active Alerts
        const activeAlerts = await (0, db_1.queryRows)('SELECT * FROM alerts WHERE user_id = ? AND acknowledged = 0 ORDER BY created_at DESC LIMIT 5', [userId]);
        // 6. Fetch Active Disaster Event
        const activeDisaster = await (0, db_1.queryRow)('SELECT * FROM disaster_events WHERE is_active = 1 ORDER BY started_at DESC LIMIT 1');
        // 7. Fetch Recent Sleep & Activity
        const recentSleep = await (0, db_1.queryRow)('SELECT * FROM sleep_records WHERE user_id = ? ORDER BY date DESC LIMIT 1', [userId]);
        const recentActivity = await (0, db_1.queryRow)('SELECT * FROM activity_records WHERE user_id = ? ORDER BY date DESC LIMIT 1', [userId]);
        ...
        let recommendations = ['Maintain hydration.', 'Continue normal activity.'];
        if (latestRisk) {
...
async function getCaregiverDashboard(req, res) {
    try {
    ...
    `;
      ...
      `;
        ...
        const rawUsers = await (0, db_1.queryRows)(userQuery);
        ...
        for (const u of rawUsers) {
            const reading = await (0, db_1.queryRow)('SELECT * FROM sensor_readings WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [u.id]);
            const risk = await (0, db_1.queryRow)('SELECT * FROM risk_assessments WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [u.id]);
            const loc = u.location_sharing
                ? await (0, db_1.queryRow)('SELECT latitude, longitude, accuracy, timestamp FROM location_records WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1', [u.id])
                : null;
            const unacknowledgedAlerts = await (0, db_1.queryRows)('SELECT * FROM alerts WHERE user_id = ? AND acknowledged = 0 ORDER BY created_at DESC', [u.id]);
            ...
            monitoredPeople.push({
                ...
                hasEmergency: unacknowledgedAlerts.some((a) => a.severity === 'CRITICAL' || a.alert_type === 'FALL_DETECTED'),
            });
        }
        // Sort: Critical first, then High, then Caution, then Low
        const severityWeight = { CRITICAL: 4, HIGH: 3, CAUTION: 2, LOW: 1 };
        ...
        const allAlerts = await (0, db_1.queryRows)(`
    ...
    `);
        ...
        const activeDisasters = await (0, db_1.queryRows)('SELECT * FROM disaster_events WHERE is_active = 1 ORDER BY started_at DESC');
        res.json({
            summary: {
