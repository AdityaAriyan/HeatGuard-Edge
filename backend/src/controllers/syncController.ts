...
export interface SyncPayload {
  userId?: string;
  readings: EdgeSensorPacket[];
  offlineRiskAssessments?: any[];
  offlineAlerts?: any[];
  offlineSosEvents?: any[];
}

export async function syncOfflineData(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const { readings = [], offlineSosEvents = [] }: SyncPayload = req.body;

    // Fetch baseline for re-evaluating or validating
    const baseline = await queryRow<{
    ...
    for (const packet of readings) {
      ...
      const risk = EdgeRiskEngine.assessRisk(packet, userBaseline, 'OFFLINE_RECONCILIATION', 'EDGE_DEVICE_LOCAL');
    ...
    for (const sos of offlineSosEvents) {
      await runQuery(
        ...
        [uuidv4(), userId, sos.latitude || null, sos.longitude || null, JSON.stringify(sos.vitalSnapshot || {}), sos.timestamp || new Date().toISOString()]
      );
    }

    // 3. Log into Sync Queue audit
    await runQuery(
      `INSERT INTO sync_queues (id, user_id, payload_type, payload_json, queued_at, status)
       VALUES (?, ?, 'SENSOR_BATCH', ?, datetime('now'), 'PROCESSED')`,
      [uuidv4(), userId, JSON.stringify({ count: processedCount })]
    );

    // Update device last sync
    await runQuery(`UPDATE devices SET last_sync_at = datetime('now') WHERE user_id = ?`, [userId]);
