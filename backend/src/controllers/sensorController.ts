...
import { broadcastSensorPacket, broadcastAlert } from '../websocket/server';

export async function ingestSensorReading(req: AuthRequest, res: Response): Promise<void> {
  try {
    const packet: EdgeSensorPacket = req.body;
    ...
    if (packet.latitude && packet.longitude) {
      await runQuery(
        `INSERT INTO location_records (id, user_id, latitude, longitude, accuracy, source, timestamp)
         VALUES (?, ?, ?, ?, 5.0, 'GPS_GNSS', ?)`,
        [uuidv4(), targetUserId, packet.latitude, packet.longitude, timestamp]
      );
    }

    // 3. Retrieve Baseline for Risk Calculation
    let baseline = await queryRow<{
    ...
    }>('SELECT * FROM baselines WHERE user_id = ?', [targetUserId]);
    ...
    const risk = EdgeRiskEngine.assessRisk(packet, userBaseline, 'ACTIVE_MONITORING', 'BACKEND_CLOUD');

    // 5. Insert Risk Assessment
    const riskAssessmentId = uuidv4();
    await runQuery(
    ...
    if (risk.isFallSuspected) {
      ...
      await runQuery(
        `INSERT INTO alerts (id, user_id, alert_type, severity, title, message, sensor_context, created_at)
         VALUES (?, ?, 'FALL_DETECTED', 'CRITICAL', '⚠️ Possible Fall Detected', 'Sudden impact and inactivity pattern registered from MPU6050.', ?, ?)`,
        [alertId, targetUserId, JSON.stringify({ hr: packet.heartRate, accel: [packet.accelX, packet.accelY, packet.accelZ] }), timestamp]
      );
      broadcastAlert({
        id: alertId,
        userId: targetUserId,
...
export async function getLatestSensorReading(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const location = await queryRow(
      'SELECT latitude, longitude, timestamp FROM location_records WHERE user_id = ? ORDER BY timestamp DESC LIMIT 1',
      [userId]
    );
...
export async function getSensorHistory(req: AuthRequest, res: Response): Promise<void> {
  try {
    const userId = (req.params.userId as string) || req.user?.id;
    ...
    const history = await queryRows(
      'SELECT * FROM sensor_readings WHERE user_id = ? ORDER BY timestamp DESC LIMIT ?',
      [userId, limit]
    );
