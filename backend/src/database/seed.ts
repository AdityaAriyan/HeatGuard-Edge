...
import { getDatabase, runQuery, queryRow, execSql } from './db';
import { initSchema } from './schema';
import { SCENARIO_PRESETS, ScenarioType } from '../edge/sensors/simulatedSensors';
import { EdgeRiskEngine } from '../edge/risk-engine/EdgeRiskEngine';
...
export async function seedDatabase(): Promise<void> {
  ...
  const defaultPasswordHash = await bcrypt.hash('HeatGuard@123', salt);
  ...
  const demoAccounts = [
    ...
    // Monitored Persons under Dr. Priya Nair
    {
      ...
      scenario: 'HEAT_WAVE' as ScenarioType,
      lat: 28.5800,
      lng: 77.2300,
    },
    {
      ...
      scenario: 'DEHYDRATION' as ScenarioType,
      lat: 28.6500,
      lng: 77.1800,
    },
    {
      ...
      scenario: 'AIR_POLLUTION' as ScenarioType,
      lat: 28.6250,
      lng: 77.2150,
    },
    {
      ...
      scenario: 'FATIGUE' as ScenarioType,
      lat: 28.6050,
      lng: 77.2450,
    },
    {
    ...
    },
    {
      ...
      scenario: 'CRITICAL' as ScenarioType,
      lat: 28.5700,
      lng: 77.1950,
    },
    {
    ...
    },
    {
      ...
      scenario: 'CYCLONE' as ScenarioType,
      lat: 28.6400,
      lng: 77.2600,
    },
  ];
  ...
  for (const acc of demoAccounts) {
    ...
    await runQuery(
      `INSERT INTO users (id, email, password_hash, full_name, role, phone)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [acc.id, acc.email, defaultPasswordHash, acc.full_name, acc.role, acc.phone]
    );

    // 2. Insert User Profile
    await runQuery(
      `INSERT INTO user_profiles (id, user_id, age, gender, occupation, work_environment, known_health_conditions, emergency_contact_name, emergency_contact_phone, blood_group)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        uuidv4(),
        acc.id,
    ...
    await runQuery(
      `INSERT INTO devices (id, user_id, device_name, device_model, hardware_platform, firmware_version, status, last_sync_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [
        devId,
        acc.id,
    ...
    if (acc.id !== caregiverId && acc.id !== adminId) {
      await runQuery(
        `INSERT INTO caregiver_relationships (id, user_id, caregiver_id, status, permission_level)
         VALUES (?, ?, ?, 'ACTIVE', 'FULL')`,
        [uuidv4(), acc.id, caregiverId]
      );
    }

    // 7. Insert Location Record
    await runQuery(
      `INSERT INTO location_records (id, user_id, latitude, longitude, accuracy, source)
       VALUES (?, ?, ?, ?, 4.5, 'GPS_GNSS')`,
      [uuidv4(), acc.id, acc.lat, acc.lng]
    );

    // 8. Generate Realistic Sensor Readings & 24h Time Series
    const profile = SCENARIO_PRESETS[acc.scenario];
    const now = Date.now();
    ...
    for (let i = 24; i >= 0; i--) {
      ...
      await runQuery(
        `INSERT INTO sensor_readings (id, user_id, device_id, heart_rate, spo2, skin_temperature, ambient_temperature, humidity, systolic, diastolic, is_bp_estimated, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, activity_state, aqi, pm25, exposure_minutes, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          readingId,
          acc.id,
      ...
      const risk = EdgeRiskEngine.assessRisk(packet, userBaseline, 'DISASTER_HEAT_WAVE', 'EDGE_DEVICE_LOCAL');

      await runQuery(
        `INSERT INTO risk_assessments (id, user_id, reading_id, overall_score, overall_level, heat_risk, dehydration_risk, respiratory_risk, fatigue_risk, cardiovascular_risk, fall_risk, reasons, recommendations, is_fall_suspected, engine_location, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          uuidv4(),
          acc.id,
      ...
      if (i === 0 && (risk.overallLevel === 'HIGH' || risk.overallLevel === 'CRITICAL' || acc.scenario === 'FALL')) {
        ...
        if (acc.scenario === 'FALL') {
          await runQuery(
            `INSERT INTO emergency_events (id, user_id, event_type, status, latitude, longitude, vital_snapshot, caregiver_notified)
    ...
    for (let day = 7; day >= 1; day--) {
      ...
      await runQuery(
        `INSERT INTO sleep_records (id, user_id, date, duration_minutes, is_estimated, sleep_quality, consistency_score)
         VALUES (?, ?, ?, ?, 1, ?, ?)`,
        [uuidv4(), acc.id, dateStr, duration, duration > 400 ? 'Good' : 'Restless', duration > 400 ? 88 : 62]
      );

      await runQuery(
        `INSERT INTO activity_records (id, user_id, date, active_minutes, step_count, inactivity_minutes, strenuous_work_minutes)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [uuidv4(), acc.id, dateStr, 220 + Math.round(Math.random() * 80), 8400 + Math.round(Math.random() * 2000), 320, 95]
      );
    }
  }
  ...
  await runQuery(
    `INSERT INTO disaster_events (id, event_type, title, severity, affected_region, description, guidelines, is_active)
  ...
  await runQuery(
    `INSERT INTO environmental_readings (id, latitude, longitude, ambient_temperature, humidity, heat_index, aqi, pm25, uv_index, weather_condition)
     VALUES (?, 28.6139, 77.2090, 41.2, 68.0, 48.5, 310, 240.0, 9.2, 'Intense Heat / Smog')`,
    [uuidv4()]
  );
